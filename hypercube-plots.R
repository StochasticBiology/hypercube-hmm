...
plot.bubbles = function(stats.df, labels=NULL) {
  ...
  ## bubbles
  if(is.null(labels)) {
    g.1 = ggplot(stats.df, aes(x=order, y=feature)) + geom_point(aes(size=mean), colour="#AAAAAA") + geom_point(aes(size=sd), shape = 1, colour="#888888") + theme_classic() 
  } else {
    g.1 = ggplot(stats.df, aes(x=order, y=feature)) + geom_point(aes(size=mean), colour="#AAAAAA") + geom_point(aes(size=sd), shape = 1, colour="#888888") + scale_y_continuous(breaks=length(fitted[[3]]):1, labels=fitted[[3]]) + theme_classic() 
  }
  g.1
}

plot.bubbles2 = function(bp, labels=NULL, formatted=F) {
  message("Building bubble plot")
  if(formatted == F) {
  bp.df = data.frame()
  for(i in 1:nrow(bp)) {
  ...
  ## bubbles
  if(is.null(labels)) {
    g.1 = ggplot(bp.df, aes(x=order, y=feature)) + geom_point(aes(size=prob), colour="#AAAAAA") + theme_classic() + theme(legend.position = "none")
  } else {
    g.1 = ggplot(bp.df, aes(x=order, y=feature)) + geom_point(aes(size=prob), colour="#AAAAAA") + scale_y_continuous(breaks=length(labels):1, labels=labels) + theme_classic() + theme(legend.position = "none")
  }
  return(g.1)
}
...
if(pfg.layout == "tree") {
  g.3 = ggraph(g, layout="tree") + geom_edge_bend(aes(edge_width=exp(weight/maxw), edge_alpha = weight/maxw), strength=curvature,  arrow=arrow()) + geom_node_point() + geom_node_label(aes(label=name), nudge_x = 0.05, nudge_y=-0.05) + theme_void() + theme(legend.position = "none")
   ...
   g.3 = ggraph(g, layout="matrix") + geom_edge_bend(aes(edge_width=exp(weight/maxw), edge_alpha = weight/maxw), strength=curvature,  arrow=arrow()) + geom_node_point() + geom_node_label(aes(label=name), nudge_x = 0.05, nudge_y=-0.05) + theme_void() + theme(legend.position = "none")
} else {
...
plot.hypercube2 = function(translist, use.width = T,           # use line width to display edge weights?
                          duplicate.offset = 0.,   # vertical offset for nodes in identical positions
                          lab.size = 3,            # size for edge labels
                          p.size = 1,              # point size
                          node.labels = T,         # node labels, yes or no?
 ...
 nodes = all.nodes
  ...
  if(use.width) {
    cube.plot = ggplot() +
      geom_segment(data=segments, aes(x=src.z, y=src.x, xend=dest.z, yend=dest.x, size=count/max(count)), color="#AAAAAA") +
      scale_size_identity() +
      geom_point(data = coords[safe.nodes == T,], aes(x=z, y=x), size=p.size, color="#AAAAAA") +
      xlim(-1,1) + ylim(-1,1) + theme_void() + theme(legend.position="none")
